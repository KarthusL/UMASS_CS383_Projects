...
class DecisionTree:
    ...
    def DTL(self, examples, attribute_names, default=True):
        ...
        # elif all(self.mode(examples[i]) for i in range(len(examples))) == self.mode(examples[0]):
        #     return self.mode(examples[0])
        # elif not attribute_names:
        #     return self.mode(examples)
        # else:
        #
        #     next = self.chooseAttribute(attribute_names, examples)
        #     tree = self.root(next)
        #     for value in next:
        #         temp = [ex for ex in examples if next == value]
        #         subtree = self.DTL(temp, next.attribute_names, self.mode(examples))
        #         tree.add(temp, value, subtree)
        #     return tree
    ...
    def mode(self, answers):
        ...
        # WRITE the required CODE HERE and return the computed values
        temp = answers
        return sorted(temp ,key = temp.count,reverse=True)[0]
