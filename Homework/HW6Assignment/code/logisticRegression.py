...
class LogisticRegression:
    ...
    def l(self, y, z):
        return -y * math.log(z, 2) - (1 - y) * math.log(1 - z, 2)
    ...
    def loss(self, w, X, Y):
        ...
        Z = np.dot(X, w)
        summation = sum(self.l(Y[i], self.sigmoid(Z[i])) for i in range(len(Y)))
    ...
    def loss_grad(self, w, X, y):
        ...
        """
        # WRITE the required CODE HERE and r®eturn the computed values
        Z = np.dot(X, self.w)
        return np.dot((self.sigmoid(Z) - y), X)
